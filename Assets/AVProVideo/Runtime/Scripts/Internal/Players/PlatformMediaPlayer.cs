...
namespace RenderHeads.Media.AVProVideo
{
	public sealed partial class PlatformMediaPlayer : BaseMediaPlayer
	{
		...
		private void PlatformMediaPlayerInitWithOptions(MediaPlayer.OptionsAndroid options)
		{
			...
			if (options.allowUnsupportedVideoTrackVariants)
				_playerSettings.videoFlags |= Native.AVPPlayerVideoOutputSettingsFlags.AllowUnsupportedVideoTrackVariants;
	...
	public sealed partial class PlatformMediaPlayer
	{
		...
		private void UpdatePlayerSettingsFromOptions(MediaPlayer.OptionsAndroid options)
		{
			...
			if (options.HasChanged(MediaPlayer.OptionsAndroid.ChangeFlags.AllowUnsupportedVideoTrackVariants))
			{
				_playerSettings.videoFlags &= ~Native.AVPPlayerVideoOutputSettingsFlags.AllowUnsupportedVideoTrackVariants;
				if (options.allowUnsupportedVideoTrackVariants)
				{
	...
	public sealed partial class PlatformMediaPlayer
	{
		...
		internal override Variant InternalGetVariantAtIndex(int index)
		{
			if (index >= 0 && index < _variantInfo.Length)
			{
				...
				VariantFlags flags = (VariantFlags)( (int)( _variantInfo[index].flags ) );
				// Debug.Log($"InternalGetVariantAtIndex({index}) - width: {width}, height: {height}, peakDataRate: {peakDataRate}, averageDateRate: {averageDataRate}, frameRate: {frameRate}, videoRange: {videoRange}, audioCodecType: {audioCodecType}, flags: {flags}");
				return new Variant(index, width, height, peakDataRate, averageDataRate, videoCodecType, frameRate, videoRange, audioCodecType, flags);
			}
			else
			{
				return null;
